...
<!-- ------------------------------------------------------------------------------------------------------------------------ -->
	            ...
	            <div class="col-md-10 col-md-offset-4">
	                ...
	                --%>
	                <button type="button" onclick="confirmAction()" class="btn btn-material-purple" style="font-weight: bold; font-size: 22px;">ลงทะเบียน</button>
	            </div>
	        </div> 
...
function confirmAction(){
	var txt;
	var r = confirm(" กรุณาตรวจสอบข้อมูล และ email ของท่าน. \n ยืนยันการสมัคร ?");
	if (r == true) {
		doAction()
	} else {
...
function doAction(){
    ...
    if ($.trim($('input[id="bizclubRegisterM.brFirstName"]').val()).length == 0) {
        alert('กรุณากรอก ชื่อ');
        $('input[id="bizclubRegisterM.brFirstName"]').focus();
        return false;
    }
    if ($.trim($('input[id="bizclubRegisterM.brLastName"]').val()).length == 0) {
        alert('กรุณากรอก นามสกุล');
        $('input[id="bizclubRegisterM.brLastName"]').focus();
        return false;
    }
    if ($.trim($('input[id="bizclubRegisterM.dateOfBirthStr"]').val()).length == 0) {
        alert('กรุณากรอก วัน เดือน ปีเกิด');
        $('input[id="bizclubRegisterM.dateOfBirthStr"]').focus();
        return false;
    }
    if ($.trim($('input[id="bizclubRegisterM.mobilePhone"]').val()).length == 0) {
        alert('กรุณากรอก วัน เบอร์โทรศัพท์ (มือถือ)');
        $('input[id="bizclubRegisterM.mobilePhone"]').focus();
        return false;
    }
    ...
    if ($.trim($('input[id="bizclubRegisterM.addressNo"]').val()).length == 0) {
        alert('กรุณากรอก ที่อยู่ที่สามารถติดต่อได้');
        $('input[id="bizclubRegisterM.addressNo"]').focus();
        return false;
    }
